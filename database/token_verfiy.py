...
async def check_user_access(client, message):
    ...
    if command and "VERIFY" in command:
        ...
        if token is None or token_value != token:
            ...
            await message.reply_textf"Invalid or expired link. Verify again: {verification_link}\n{TUTORIAL_VIDEO}")
            return False
    ...
    if verified_at is None or (time.time() - verified_at > TOKEN_VERIFY_TIME):
        ...
        await db.update_user(user_id, {'token': new_token})
        verification_link = await convert_short_link(f'https://telegram.me/{CONFIG_DICT['BOT_USERNAME']}?start=VERIFY-{new_token}')
        await message.reply_textf"Verification required again: {verification_link}\n{TUTORIAL_VIDEO}")
        return False
