...
async def start_command(client: Client, message: Message):
    ...
    if not await present_user(id):
        try:
            ...
            pass
    if not await check_user_sub_status(client, message):
        return
    ...
    if len(text)>7:
        ...
        CUSTOM_CAPTION = CONFIG_DICT['CUSTOM_CAPTION']
        for msg in messages:
            ...
            try:
                file_msg = await msg.copy(chat_id=message.from_user.id, caption = caption, parse_mode = ParseMode.HTML, reply_markup = reply_markup, protect_content=CONFIG_DICT['PROTECT_CONTENT'])
        ...
        await message.reply_text(
            text = CONFIG_DICT['START_MSG'].format(
...
@Bot.on_message(filters.command('users') & filters.private & filters.user(CONFIG_DICT['ADMINS']))
async def get_users(client: Bot, message: Message):
    msg = await client.send_message(chat_id=message.chat.id, text=WAIT_MSG)
    users = await full_userbase()
    await msg.edit(f"{len(users)} users are using this bot")

@Bot.on_message(filters.private & filters.command('broadcast') & filters.user(CONFIG_DICT['ADMINS']))
